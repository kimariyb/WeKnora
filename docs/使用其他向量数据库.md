...
`DB_DRIVER=mysql` 只表示 WeKnora 的主业务数据库使用 MySQL。MySQL 不承接 PostgreSQL / ParadeDB 的 pgvector 检索能力，因此不能与 `RETRIEVE_DRIVER=postgres` 搭配使用。

使用 MySQL 主数据库时，请将 `RETRIEVE_DRIVER` 设置为外部向量库驱动，例如：
...
可选驱动包括 Qdrant、Milvus、Weaviate、Apache Doris、Tencent VectorDB、Elasticsearch 和 OpenSearch。具体连接参数请参考 `.env.example` 中对应驱动的配置项。
