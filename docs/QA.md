...
在 `.env` 中设置 `DB_DRIVER=mysql`，并将 `DB_HOST`、`DB_PORT`、`DB_USER`、`DB_PASSWORD`、`DB_NAME` 指向 MySQL 8.x 实例。开发模式可使用：
...
MySQL 只作为主业务数据库，不提供 PostgreSQL / ParadeDB 的 pgvector 检索能力。因此 `DB_DRIVER=mysql` 不能搭配 `RETRIEVE_DRIVER=postgres`，需要使用 Qdrant、Milvus、Weaviate、Doris、Tencent VectorDB、Elasticsearch 或 OpenSearch 等外部向量库。
