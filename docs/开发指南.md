...
`DB_DRIVER=mysql` 只切换主业务数据库。MySQL 不提供 pgvector / ParadeDB 检索能力，因此不能搭配 `RETRIEVE_DRIVER=postgres`；请使用 Qdrant、Milvus、Weaviate、Doris、Tencent VectorDB、Elasticsearch 或 OpenSearch 等外部向量库。
