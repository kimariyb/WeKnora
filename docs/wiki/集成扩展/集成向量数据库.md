...
`DB_DRIVER=mysql` 只切换 WeKnora 主业务数据库，不提供 PostgreSQL / ParadeDB 的 pgvector 检索能力。因此 MySQL 主数据库不能搭配 `RETRIEVE_DRIVER=postgres`，需要使用 Qdrant、Milvus、Weaviate、Apache Doris、Tencent VectorDB、Elasticsearch 或 OpenSearch 等外部向量库。
